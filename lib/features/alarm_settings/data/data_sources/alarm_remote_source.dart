...
abstract class AlarmRemoteSource {
  Future<AlarmModel> getAlarmSetting({required Map<String, dynamic> urlData});
  Future<void> publishMqttCommand({required String controllerId, required String command});
  Future<void> saveAlarmSettings({required String endpoint, required Map<String, dynamic> body});
...
class AlarmRemoteSourceImpl implements AlarmRemoteSource {
  final ApiClient apiClient;
  AlarmRemoteSourceImpl({required this.apiClient});

  @override
  Future<AlarmModel> getAlarmSetting({required Map<String, dynamic> urlData}) async {
    try {
      String endPoint = buildUrl(AlarmUrls.alarmSetting, urlData);
      final response = await apiClient.get(endPoint);
      if (response['code'] == 200 && response['data'] != null && (response['data'] as List).isNotEmpty) {
        return AlarmModel.fromJson(response['data'][0]);
      }
      throw Exception('Failed to fetch alarm setting');
    } catch (e) {
      rethrow;
    }
  }
  ...
  @override
  Future<void> saveAlarmSettings({required String endpoint, required Map<String, dynamic> body}) async {
    try {
